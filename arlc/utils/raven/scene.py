...
class SceneEngine(object):
    def __init__(self, number_slots, device):
        ...
        for i in range(start_index, self.num_slots + 1): self.num_pos_index_map[i] = torch.tensor(list(filter(lambda idx: position2number[idx] == i,range(len(position2number)),)),dtype=torch.long,).to(self.device)
    ...
    def compute_scene_prob(self, exist_logprob, type_logprob, size_logprob, color_logprob):
        ...
        return (SceneProb(position_prob, number_prob, type_prob, size_prob, color_prob),SceneLogProb(position_logprob,utils.log(number_prob),utils.log(type_prob),utils.log(size_prob),utils.log(color_prob),),)
    ...
    def compute_position_prob(self, exist_logprob):
        ...
        index = self.positions.unsqueeze(0).unsqueeze(0).expand(batch_size, num_panels, -1, -1).unsqueeze(-1).type(torch.long)
        position_logprob = torch.gather(exist_logprob, -1, index)
        position_logprob = torch.sum(position_logprob.squeeze(-1), dim=-1)  # (batch_size, num_panels, self.dim_position)
    ...
    def compute_type_prob(self, type_logprob, position_logprob):
        ...
        num_panels = type_logprob.shape[1]
        index = self.positions.unsqueeze(0).unsqueeze(0).expand(batch_size, num_panels, -1, -1).unsqueeze(-1).type(torch.float)
        type_logprob = type_logprob.unsqueeze(2).expand(-1, -1, self.dim_position, -1, -1)
        type_logprob = index * type_logprob
    ...
    def compute_color_prob(self, color_logprob, position_logprob):
        ...
        num_panels = color_logprob.shape[1]
        index = self.positions.unsqueeze(0).unsqueeze(0).expand(batch_size, num_panels, -1, -1).unsqueeze(-1).type(torch.float)
        color_logprob = color_logprob.unsqueeze(2).expand(-1, -1, self.dim_position, -1, -1)
        color_logprob = index * color_logprob  # (batch_size, num_panels, self.dim_position, slots, DIM_COLOR)
