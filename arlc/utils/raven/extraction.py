...
parser = argparse.ArgumentParser(description="NVSA")
parser.add_argument("--data_path", type=str, default="/dccstor/saentis/data/I-RAVEN")

pos_num_rule_idx_map_four = {
    "Constant": 0,
...
type_idx_rule_map = {
    ...
    3: "Progression_Two",
    4: "Progression_Mtwo",
    5: "Distribute_Three_Left",
    6: "Distribute_Three_Right",
}
size_idx_rule_map = {
    0: "Constant",
    1: "Progression_One",
    ...
    5: "Arithmetic_Plus",
    6: "Arithmetic_Minus",
    7: "Distribute_Three_Left",
    8: "Distribute_Three_Right",
}
color_idx_rule_map = {
    0: "Constant",
    1: "Progression_One",
...
pos_num_rule_four_idx_map = {
    0: "Constant",
    1: "Progression_One_Pos",
    2: "Progression_Mone_Pos",
    3: "Arithmetic_Plus_Pos",
    4: "Arithmetic_Minus_Pos",
    5: "Distribute_Three_Left_Pos",
    6: "Distribute_Three_Right_Pos",
    7: "Progression_One_Num",
    8: "Progression_Mone_Num",
    9: "Arithmetic_Plus_Num",
    10: "Arithmetic_Minus_Num",
    11: "Distribute_Three_Left_Num",
    12: "Distribute_Three_Right_Num",
}

pos_num_rule_nine_idx_map = {
    0: "Constant",
    1: "Progression_One_Pos",
    2: "Progression_Mone_Pos",
    3: "Progression_Two_Pos",
    4: "Progression_Mtwo_Pos",
    5: "Arithmetic_Plus_Pos",
    6: "Arithmetic_Minus_Pos",
    7: "Distribute_Three_Left_Pos",
    8: "Distribute_Three_Right_Pos",
    9: "Progression_One_Num",
    10: "Progression_Mone_Num",
    11: "Progression_Two_Num",
    12: "Progression_Mtwo_Num",
    13: "Arithmetic_Plus_Num",
    14: "Arithmetic_Minus_Num",
    15: "Distribute_Three_Left_Num",
    16: "Distribute_Three_Right_Num",
}


def get_pos_num_rule(
    rule_idx, comp_idx, num_elements, pos_num_rule_idx_map, xml_panels, xml_rules
):
    ...
    if index_name == "Progression":
        ...
        if attrib_name == "Pos":
            ...
            if (
                len(
                    set(map(lambda index: (index + 1) % num_elements, first))
                    - set(second)
                )
                == 0
                and len(
                    set(map(lambda index: (index + 1) % num_elements, second))
                    - set(third)
                )
                == 0
                and len(
                    set(map(lambda index: (index + 1) % num_elements, fourth))
                    - set(fifth)
                )
                == 0
                and len(
                    set(map(lambda index: (index + 1) % num_elements, fifth))
                    - set(sixth)
                )
                == 0
                and len(
                    set(map(lambda index: (index + 1) % num_elements, seventh))
                    - set(eighth)
                )
                == 0
            ):
            ...
            if (
                len(
                    set(map(lambda index: (index + 2) % num_elements, first))
                    - set(second)
                )
                == 0
                and len(
                    set(map(lambda index: (index + 2) % num_elements, second))
                    - set(third)
                )
                == 0
                and len(
                    set(map(lambda index: (index + 2) % num_elements, fourth))
                    - set(fifth)
                )
                == 0
                and len(
                    set(map(lambda index: (index + 2) % num_elements, fifth))
                    - set(sixth)
                )
                == 0
                and len(
                    set(map(lambda index: (index + 2) % num_elements, seventh))
                    - set(eighth)
                )
                == 0
            ):
...
def main():
    ...
    save_name_list = [
        "center_single_extracted_with_rules",
        "distribute_four_extracted_with_rules",
        "distribute_nine_extracted_with_rules",
        "left_right_extracted_with_rules",
        "up_down_extracted_with_rules",
        "in_out_single_extracted_with_rules",
        "in_out_four_extracted_with_rules",
    ]
    ...
    my_bbox = {
        "in_out_four_extracted_with_rules": {
            "[0.5, 0.5, 1, 1]": 0,
            "[0.42, 0.42, 0.15, 0.15]": 1,
            "[0.42, 0.58, 0.15, 0.15]": 2,
            "[0.58, 0.42, 0.15, 0.15]": 3,
            "[0.58, 0.58, 0.15, 0.15]": 4,
        },
        "in_out_single_extracted_with_rules": {
            "[0.5, 0.5, 1, 1]": 0,
            "[0.5, 0.5, 0.33, 0.33]": 1,
        ...
        "distribute_nine_extracted_with_rules": {
            "[0.16, 0.16, 0.33, 0.33]": 0,
            "[0.16, 0.5, 0.33, 0.33]": 1,
            "[0.16, 0.83, 0.33, 0.33]": 2,
            "[0.5, 0.16, 0.33, 0.33]": 3,
            "[0.5, 0.5, 0.33, 0.33]": 4,
            "[0.5, 0.83, 0.33, 0.33]": 5,
            "[0.83, 0.16, 0.33, 0.33]": 6,
            "[0.83, 0.5, 0.33, 0.33]": 7,
            "[0.83, 0.83, 0.33, 0.33]": 8,
        },
        "distribute_four_extracted_with_rules": {
            "[0.25, 0.25, 0.5, 0.5]": 0,
            "[0.25, 0.75, 0.5, 0.5]": 1,
    ...
    for w in range(len(constellation_name_list)):
        ...
        for n in range(len(obj_name_list)):
            ...
            for j in tqdm.tqdm(range(10001)):
                try:
                    tree = ET.parse(
                        "{0}/{1}/RAVEN_{2}_{3}.xml".format(
                            DATA_PATH, file_type, j, obj_name
                        )
                    )
                except:
                    continue
                ...
                for panel in root[0]:
                    ...
                    for component in panel[0]:
                        for entity in component[0]:
                            ...
                            pos = (
                                my_bbox[save_name][bbox]
                                if save_name != "center_single_extracted_with_rules"
                                else 0
                            )
                            ...
                            ext_panel = (
                                ext_comp
                                if (idx == 0)
                                else np.concatenate((ext_panel, ext_comp), axis=0)
                            )
                            idx = idx + 1
                    c = 9 - idx
                    if c > 0:
                    ...
                    ext_sample = (
                        ext_panel
                        if (idx_panel == 0)
                        else np.concatenate((ext_sample, ext_panel), axis=0)
                    )
                    idx_panel = idx_panel + 1
                file = np.load(
                    "{0}/{1}/RAVEN_{2}_{3}.npz".format(
                        DATA_PATH, file_type, j, obj_name
                    )
                )
                filename = "{0}/{1}/{2}/RAVEN_{3}_{2}.npz".format(
                    DATA_PATH, save_name, obj_name, count
                )
                np.savez(
                    filename,
                    target=file["target"],
                    ...
                    image=file["image"],
                    meta_matrix=file["meta_matrix"],
                    meta_structure=file["meta_structure"],
                    meta_target=file["meta_target"],
                    structure=file["structure"],
                    extracted_meta=ext_sample,
                    rules=rules,
                )
                count = count + 1
