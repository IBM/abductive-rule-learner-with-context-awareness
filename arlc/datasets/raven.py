...
class RAVENDataset(Dataset):
    def __init__(self,dataset_type,data_dir,constellation_filter,rule_filter="",attribute_filter="",n_train=None,in_memory=False,maxval=-1,partition=""):
        ...
        if self.in_memory:
            self.dataset = []
            for idx in tqdm(self.filtered_indeces):
                data_path = os.path.join(self.extracted_path,"RAVEN_" + str(idx) + "_" + self.dataset_type + ".npz",)
                data = np.load(data_path)
                self.dataset.append(data)

    def rule_idx(self, rule_name, attribute_name):
        if attribute_name == "Position":
            if self.constellation_filter == "distribute_four":rule_idx_map = {"Constant": [0], "Progression": [1, 2], "Arithmetic": [3, 4], "Distribute_Three": [5, 6]}
            if self.constellation_filter == "distribute_nine":rule_idx_map = {"Constant": [0], "Progression": [9, 10, 11, 12], "Arithmetic": [13, 14], "Distribute_Three": [15, 16]}
        elif attribute_name == "Number":
            if self.constellation_filter == "distribute_four":rule_idx_map = {"Constant": [0], "Progression": [7, 8], "Arithmetic": [9, 10], "Distribute_Three": [11, 12]}
            if self.constellation_filter == "distribute_nine":rule_idx_map = {"Constant": [0], "Progression": [9, 10, 11, 12], "Arithmetic": [13, 14], "Distribute_Three": [15, 16]}
        elif attribute_name == "Type":rule_idx_map = {"Constant": [0], "Progression": [1, 2, 3, 4], "Distribute_Three": [5, 6]}
        elif attribute_name == "Size":rule_idx_map = {"Constant": [0], "Progression": [1, 2, 3, 4], "Arithmetic": [5, 6], "Distribute_Three": [7, 8]}
        elif attribute_name == "Color":rule_idx_map = {"Constant": [0], "Progression": [1, 2, 3, 4], "Arithmetic": [5, 6], "Distribute_Three": [7, 8]}
